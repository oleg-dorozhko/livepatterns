...
function mn_fillRectangleFast(imgData2, x, y, n, m, col )
{
...
function nineth(im)
{
	...
	var newpng = ctx.getImageData(0,0,cnv.width,cnv.height);  
	...
	for (var y = 0; y < h; y++) {
		...
		for (var x = 0; x < w; x++) {
				...
				newpng = mn_fillRectangleFast(newpng, x*3, y*3, 3, 3, avg_color );
...
function mn_getColorArrayFromImageDataByIndex(imgData0, idx)
{
...
function nonineth(im)
{
	...
	for (var y = 1; y < h; y+=3) {
		...
		for (var x = 1; x < w; x+=3) {
				...
				// var y1=y/3;
				var idx= w*y+x << 2;
				var color= mn_getColorArrayFromImageDataByIndex(im, idx);
				newpng = mn_fillRectangleFast(newpng, n, m, 1, 1, color );
				// newpng.data[ind]=avg_color[0];
				// newpng.data[ind+1]=avg_color[1];
				// newpng.data[ind+2]=avg_color[2];
				// newpng.data[ind+3]=255;
