function mirror_right (dt)
{
	...
	context.putImageData(dt,0,0);
		...
		// {
			...
			// console.log("mod_mright: error: too big size (need result width * 2 <= 1200 || height <= 1200)");
			// return;
		...
		var im = context2.getImageData(0,0,canvas2.width,canvas2.height);
			...
			***/
			//setTimeout( function(){ 	logg('mirror right'); //after or before? what question
			//}, 100 );	
...
function mirror_right2 (dt)
{
	
	var im = mirror_right (dt);
	return mod_cli_up_getPartOfImageData(im,dt.width,0,dt.width,im.height);
}