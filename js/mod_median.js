...
function median(im)
{
	...
	var nx = (im.width/4|0);
	var ny = (im.height/4|0);
	
	
	return context.getImageData(nx,ny,(im.width/2|0),(im.height/2|0));
/********************* for oddd -----------
...
****************************************/
	...
	newpng.width = xlength*2;
	newpng.height = ylength*2;
	var context = newpng.getContext("2d");
	newpng = newpng.getContext("2d").getImageData(0,0,xlength*2,ylength*2);
	//var w = newpng.width;
	//var h = newpng.height;
		...
		var m=0;
		var n=0;
			for (var y = y0; y < ylength*2-newpng.height/4; y++) {
				n=0;
				for (var x = x0; x < xlength*2-newpng.width/4; x++) {
					var idx = (w * y + x) << 2;
	...
	return  __half_imageData(newpng);//.getContext("2d").getImageData(0,0,w,h);
